...
public class MuObject {
	 ...
	 */
	protected Map<Integer, MuObject> _knownObjects = new FastMap()
			.setShared(true);
	/**
	 * aet of players knowns
	 */
	private final Set _knownPlayer = new HashSet();
	...
	public void addKnownObjects(Vector<MuObject> obj) {
		...
		for (int i = 0; i < obj.size(); i++) {
			...
			if (obj.get(i) instanceof MuPcInstance
					|| obj.get(i) instanceof MuPcActorInstance) {
	...
	public void removeAllKnownObjects() {
		final MuObject[] notifyList = _knownObjects.values().toArray(
				new MuObject[_knownObjects.size()]);
		// clear our own list
		_knownObjects.clear();

		for (final MuObject element : notifyList) {
			element.removeKnownObject(this, RemKnow_NoRelasion);
		}
	}
	 ...
	 * 
	 * @param object
	 *            to removefrom knownlist
	 * @param why
	 *            remove object [swnd specifed packages in pcinstance]:
	...
	public void ShowNyKnowList() {
		...
		final Collection oldlist = oldgetKnownObjects().values();
		System.out.println("KnowList of " + this);
		int i = 0;
		for (final Object knowns : oldlist) {
			i++;
