...
//23 ad 7f 0c
    ...
    public CMoveItemRequest(byte[] decrypt, ClientThread _client) {
        ...
        System.out.println("Move Item Request : from wid[" + _windowFrom + "] slot[" + _slotFrom + "] to " +
        ...
        if (!_result) {
            _client.getActiveChar().sendPacket(new SMoveItemResult(
                    _windowFrom, _slotFrom, _itemHex));
        } else {
            _client.getActiveChar().sendPacket(new SMoveItemResult(
                    _windowTo, _slotTo, _itemHex));
        }
    }
